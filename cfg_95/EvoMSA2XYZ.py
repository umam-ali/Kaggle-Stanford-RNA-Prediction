...
class MSA2xyzIteration(nn.Module):
    ...
    def forward(self,msa_,idx,ss_,m1_pre,z_pre,pre_x,cycle_index,alphas):
    ...
    def pred(self,msa_,idx,ss_,m1_pre,z_pre,pre_x,cycle_index,alphas):
        m1_all,z_all,s_all=0,0,0
        N,L,_=msa_.shape
...
class MSA2XYZ(nn.Module):
    ...
    def forward(self,msa_,idx,ss,true_x,base_x,alphas):
        #########################################################################
        coornorm=true_x.norm(dim=-1)
        coornorm=coornorm[:,0]*coornorm[:,1]*coornorm[:,2]
        xmask=((coornorm>0)[:,None]*(coornorm>0)[None,:]).float()
        struc_mask = (idx[:,None] - idx[None]).abs()
        #################################get mask################################
        L=msa_.shape[1]
        m1_pre,z_pre=0,0
        x_pre=torch.zeros(L,3,3).to(msa_.device)
        a=np.random.randint(self.N_cycle)+1
        #print(a)
        runtime=0
        for i in range(1,a):
            m1,z,s,_,_,_=self.msaxyzone(msa_,idx,ss,m1_pre,z_pre,x_pre,runtime,alphas)
            x = self.structurenet(s,z,true_x,xmask,base_x,struc_mask)[-1]
            m1_pre=m1.detach()
            z_pre = z.detach()
            x_pre = x.detach()
            runtime+=1
        ########################last cycle###########
        m1,z,s,m,msa,msa_mask=self.msaxyzone(msa_,idx,ss,m1_pre,z_pre,x_pre,runtime,alphas)
        losses,side_losses,final_loss,rmsdloss,rmsdloss_final,_,_,x = self.structurenet(s,z,true_x,xmask,base_x,struc_mask)
        pred_msa = F.log_softmax(self.msa_predor(m),dim=-1)
        pred_disp = F.log_softmax(self.pdis_predor(z),dim=-1)      
        pred_disc = F.log_softmax(self.cdis_predor(z),dim=-1)  
        pred_disn = F.log_softmax(self.ndis_predor(z),dim=-1)    
        msaloss = 0#loss.mask_msa_loss(msa,pred_msa,msa_mask)
        dislossp = loss.dis_loss(pred_disp,true_x,xmask,0)  
        dislossc = loss.dis_loss(pred_disc,true_x,xmask,1) 
        dislossn = loss.dis_loss(pred_disn,true_x,xmask,2) 
        return losses,side_losses,final_loss,rmsdloss,rmsdloss_final,msaloss,dislossp+dislossc+dislossn,x
    ...
    def pred(self,msa_,idx,ss,base_x,alphas):
        ...
        for i in range(self.N_cycle):
            m1,z,s=self.msaxyzone.pred(msa_,idx,ss,m1_pre,z_pre,x_pre,i,alphas)          
