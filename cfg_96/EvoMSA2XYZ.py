...
_DRFOLD2_ROOT_FOR_CACHE = r"/kaggle/working/DRfold2" # Or get from env var if set by main
...
class PreMSA(nn.Module):
    def __init__(self,seq_dim,msa_dim,m_dim,z_dim):
        ...
        print(f"pos{self.pos.sum().item()}")
        print(f"pos1{self.pos1d.sum().item()}")
...
class ex_zBlock(nn.Module):
    def __init__(self,z_dim):
        super(ex_zBlock,self).__init__()
        self.pair_triout=EvoPair.TriOut(z_dim)
        self.pair_triin =EvoPair.TriIn(z_dim)
        self.pair_tristart=EvoPair.TriAttStart(z_dim)
        self.pair_triend  =EvoPair.TriAttEnd(z_dim)
    ...
    def forward(self,z):
        ...
        z = z + self.pair_trans(z)
        return z
class ex_zAttention(nn.Module):
    def __init__(self, z_dim,N_head=8,c=8) -> None:
        super(ex_zAttention,self).__init__()
        self.N_head = N_head
        self.c = c
        self.sq_c = 1/math.sqrt(c)
        self.norm1=nn.LayerNorm(z_dim)
    ...
    def forward(self,z):
        ...
        z = self.olinear(o.reshape(N,L,L,-1))   
        return z
class ex_zModule(nn.Module):
    def __init__(self,in_z_dim,z_dim,N_head=8,c=8):
        super(ex_zModule,self).__init__()
        self.z_dim = z_dim
        self.emblinear = nn.Linear(in_z_dim,z_dim)
        self.block1 = zBlock(z_dim)
        #self.block2 = zBlock(z_dim)
        self.ssatt1  = ex_zAttention(z_dim)
        self.trans1  = EvoPair.PairTrans(z_dim,1)
        self.ssatt2  = ex_zAttention(z_dim)
        self.trans2  = EvoPair.PairTrans(z_dim,1)
    def batchz(self,z,bk):
        N = z.shape[0]
        slist = [bk(z[n]) for n in range(N)]
    ...
    def forward(self,ss):
        # ss N x l x l x in_z_dim
        z = self.emblinear(ss) # N x l x l  x z
        z = self.batchz(z,self.block1)
        #z = self.batchz(z,self.block2) # N x l x l  x z
        z = z + self.ssatt1(z)
        z = z + self.trans1(z)
...
class ex_sModule(nn.Module):
    def __init__(self,s_dim,m_dim):
        super(ex_sModule,self).__init__()
        self.emblinear = nn.Linear(s_dim,m_dim)
        self.msa_row=EvoMSA.MSARow_s(m_dim)
        self.msa_col=EvoMSA.MSACol(m_dim)
        self.msa_trans=EvoMSA.MSATrans(m_dim)
        self.msa_row2=EvoMSA.MSARow_s(m_dim)
        self.msa_col2=EvoMSA.MSACol(m_dim)
        self.msa_trans2=EvoMSA.MSATrans(m_dim)
    def forward(self,s):
        # ss N x l x l x in_z_dim
        s = self.emblinear(s) # N x l x l  x z
        s = s + self.msa_row(s)
        s = s + self.msa_col(s)
        s = s + self.msa_trans(s)
        s = s + self.msa_row2(s)
        s = s + self.msa_col2(s)
        s = s + self.msa_trans2(s)
        return s[-1]
...
class RecyclingPoolEmbedder(nn.Module):
    def __init__(self,m_dim,s_dim,z_dim,dis_encoding_dim,dis_dim):
        super(RecyclingPoolEmbedder,self).__init__()  
        self.linear = basic.Linear(dis_encoding_dim*2+1,z_dim)
        self.dis_encoding_dim=dis_encoding_dim
        self.normz = nn.LayerNorm(z_dim)
        self.normm = nn.LayerNorm(m_dim)
        self.NN_dim = dis_dim 
        self.z_pool_att = ex_zModule(in_z_dim = z_dim,z_dim = z_dim)
        self.z_pool_ss  = ex_zModule(in_z_dim = 2,z_dim = z_dim)
        self.z_pool_dis = ex_zModule(in_z_dim = self.NN_dim,z_dim = z_dim)
        self.z_pool_hb = ex_zModule(in_z_dim = 6,z_dim = z_dim)
        self.z_pool_x = ex_zModule(in_z_dim = dis_encoding_dim*2+1,z_dim = z_dim)
        self.s_pool   = ex_sModule(s_dim = s_dim,m_dim=m_dim)


        self.z_linear_pre = basic.Linear(z_dim,z_dim)
        self.ss_linear_pre = basic.Linear(2,z_dim)
        self.dis_linear_pre = basic.Linear(self.NN_dim,z_dim)
        self.x_linear_pre = basic.Linear(dis_encoding_dim*2+1,z_dim)
        self.s_linear_pre = basic.Linear(s_dim,m_dim)


    def forward(self,previous_s,previous_z,previous_dis,previous_x,previous_hb):

        m1,z = 0,0
        if len(previous_z) > 0:
            # 
            #embedded_z = self.z_pool_att( torch.stack(previous_z) )
            #embedded_z = self.z_linear_pre(previous_z[-1])
            embedded_z = checkpoint(self.z_pool_att,torch.stack(previous_z))
            z = z + self.normz(embedded_z)
        # if len(previous_ss) > 0:
        #     #embedded_ss = self.z_pool_ss( torch.stack(previous_ss) )
        #     embedded_ss = checkpoint(self.z_pool_ss,torch.stack(previous_ss))
        #     #embedded_ss = self.ss_linear_pre(previous_ss[-1])
        #     z = z + self.normz(embedded_ss)
        if len(previous_dis) > 0:
            #embedded_dis = self.z_pool_dis( torch.stack(previous_dis) )
            embedded_dis = checkpoint(self.z_pool_dis,torch.stack(previous_dis))

            #embedded_dis = self.dis_linear_pre(previous_dis[-1])
            z = z + self.normz(embedded_dis)

        if len(previous_hb) > 0:
            #embedded_dis = self.z_pool_dis( torch.stack(previous_dis) )
            embedded_hb = checkpoint(self.z_pool_hb,torch.stack(previous_hb))

            #embedded_dis = self.dis_linear_pre(previous_dis[-1])
            z = z + self.normz(embedded_hb)

        if len(previous_x) >0:
            cbs = [x[:,-1] for x in previous_x]
            dismaps = [(cb[:,None,:]-cb[None,:,:]).norm(dim=-1) for cb in cbs]
            dis_zs = [fourier_encode_dist(dismap,self.dis_encoding_dim) for dismap in dismaps]
            #embedded_x = self.z_pool_x(  torch.stack(dis_zs) )
            embedded_x = checkpoint(self.z_pool_x,torch.stack(dis_zs))

            #embedded_x = self.x_linear_pre(dis_zs[-1])
            z = z + self.normz(embedded_x)
        if len(previous_s) >0:
            #embedded_s = self.s_pool( torch.stack(previous_s)  )
            embedded_s = checkpoint(self.s_pool,torch.stack(previous_s))

            #embedded_s =self.s_linear_pre( previous_s[-1])
            m1 = m1 + self.normm(embedded_s)
...
class MSA2xyzIteration(nn.Module):
    ...
    def pred(self,msa_,idx,ss_,m1_pre,z_pre,pre_x,cycle_index,alphas,previous_s,previous_z,previous_dis,previous_x,previous_hb):
        ...
        for i in range(self.N_ensemble):
            ...
            z = z+z_+ex_z
            m=torch.cat([(m[0]+m1_+ex_s)[None,...],m[1:]],dim=0)
...
class MSA2XYZ(nn.Module):
    ...
    def pred(self,msa_,idx,ss,base_x,alphas):
        ...
        previous_s,previous_z,previous_hb,previous_dis,previous_x = [],[],[],[],[]
        ret = {}
        for i in range(self.N_cycle):
            m1,z,s=self.msaxyzone.pred(msa_,idx,ss,m1_pre,z_pre,x_pre,i,alphas,previous_s,previous_z,previous_dis,previous_x,previous_hb)
            ...
            previous_s.append(s.detach())
            previous_z.append(z.detach())
            previous_dis.append(pred_disn.detach())
            previous_x.append(x.detach())
            previous_hb.append(pred_hb.detach())
            if i == self.N_cycle - 1:
